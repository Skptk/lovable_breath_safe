...
- **Atmospheric Overlay Simplification**: Removed the floating particle overlay by deleting `src/components/AirParticles.tsx` and stripping its usage from `src/pages/Index.tsx`, `src/pages/Demo.tsx`, and `src/components/index.ts`, ensuring the app renders without ambient particle effects.
...
- **Dashboard Initial Load Guardrails**: Prevent overlapping AQI refresh chains in `src/hooks/useAirQuality.ts`, skip redundant global environmental refetches in `src/hooks/useGlobalEnvironmentalData.ts`, and defer `src/components/SmokeEffect.tsx` rendering until after first paint to reduce first-load stalls.
- **Refresh Lock Persistence**: Ensure `src/hooks/useAirQuality.ts` honours the 15-minute refresh lock when the dashboard remounts, preventing duplicate history inserts after navigation loops.
- **Live AQI Guardrails**: Block the live `fetchAQI` fallback in `src/hooks/useAirQuality.ts` until scheduled data finishes loading and skip history inserts when the live timestamp isn’t newer than the scheduled record, keeping the dashboard anchored to cron data.
- **Scheduled AQI Unification**: Normalized `supabase/functions/fetchAQI/index.ts`, `src/hooks/useAirQuality.ts`, `src/components/HistoryView.tsx`, `src/components/MapView.tsx`, and `src/components/WeatherStats.tsx` so all live fetches reuse the cron-collected AQICN readings, eliminating source flips and schema mismatches.
...
- **Floating Particle Shutdown**: Removed the `AirParticles` overlay, eliminating background `requestAnimationFrame` loops and pointer listeners to lower idle CPU/GPU usage.
